 ...
 */
  ...
  async upgradeAdmin(usernamee: string) {
    await this.AdminLink.click();
    await this.AddButton.click();
    await this.userRole.nth(0).click();
    const adminRoleOption = this.page.locator('//div[@role= "listbox"]/div[3]');
    ...
    await this.empNameSubmit.click();
    await this.page.waitForTimeout(3000);
    const essRow = this.page.locator("//div[contains(@class,'oxd-table-row') and contains(. , 'ESS')]");
    await essRow.locator("//i[contains(@class,'oxd-icon bi-pencil-fill')]").nth(0).click();
    await this.userRole.nth(0).click();
    ...
    const tab = await this.page.locator(`//div[contains(@class,'oxd-table-row')and contains(. , '${usernamee}')]/div[3]`).innerText();
